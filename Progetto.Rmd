...
train.balanced.tiny = sample_frac(acc.balanced.tiny, 0.7)
test.balanced.tiny = anti_join(acc.balanced.tiny, train.balanced.tiny)

fit.rf.bestMtry = tuneRF(train.balanced.tiny %>% select(-y), train.balanced.tiny$y, ntreeTry = 100, doBest=T)

varImpPlot(fit.rf.bestMtry, type=1)
```


```{r}
fit.rf.bestMtry = tuneRF(train.balanced.tiny %>% select(-y), train.balanced.tiny$y, ntreeTry = 200, doBest=T)
