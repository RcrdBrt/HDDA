...
```{r}
acc.balanced1k <- acc %>% group_by(y) %>% sample_n(1e3) %>% ungroup()

train.balanced1k <- sample_frac(acc.balanced1k, 0.8)
test.balanced1k <- anti_join(acc.balanced1k,train.balanced1k)

fit.balanced1k <- glm(y~., train.balanced1k %>% select(-), family="binomial")

yhat.balanced1k <- predict(fit.balanced1k, test.balanced1k)

ytest.balanced1k <- test.balanced1k$y

InformationValue::confusionMatrix(ytest.balanced1k, yhat.balanced1k)
```
