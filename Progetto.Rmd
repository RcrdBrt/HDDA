...
plot_precision_at_k <- function(actuals, predictions){
...
plot_precision_vs_npred(test$y, list("glmunbalanced"=yhat.glm.unbalanced, "glmundersampling"=yhat.glm.under, "glmweighted"=yhat.glm.weighted, "glmoverweighted"=yhat.glm.overweighted), title = "Results")
```
...
```{r}
plot_precision_vs_npred(test$y, list("glmunbalanced"=yhat.glm.unbalanced, "glmundersampling"=yhat.glm.under, "glmweighted"=yhat.glm.weighted), title = "Results")
```
...
```{r}
model_base <- coxph(Surv(actual_duration_to_term, status) ~ ., data = train_surv)
...
```{r}
ph_model <- coxph(Surv(actual_duration_to_term, status) ~ 0+., train_ph, x=T,y=T)
ph_formula <- paste(names(ph_model$coefficient), collapse = "+")

model <- coxph(Surv(actual_duration_to_term, status) ~
                 dti+
                 delinq_2yrs+
                 emp_length+
                 loan_amnt+
...
```{r}
score <- Score(list("PH model"=ph_model,"Stratified Model"=model),
...
```{r}
fit.cox<-survfit(model,newdata=test_surv)
yhat.cox <- as.numeric(summary(fit.cox,times=1)$surv)

InformationValue::misClassError((1-test_surv$status), yhat.cox)

MyConfusionMatrix(as.factor(as.character(1-test_surv$status)), yhat.cox, threshold = 0.5)
```
