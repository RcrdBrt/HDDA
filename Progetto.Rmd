...
MyConfusionMatrix <- function(actuals,predictions,threshold=0.5, title=""){
  ...
  ggplot(as.data.frame(cm$table), aes(Actual, Prediction, fill= Freq)) +
    ...
    labs(x = "Actual",y = "Prediction", title=title, subtitle=paste("Precision at 0.5 threshold:", round(precision(actuals,predictions,threshold),3)*100,"%")) +
...
cat("Accuracy of Unbalanced Model:", 1-InformationValue::misClassError(test$y, yhat.glm.unbalanced))
```
...
cat("Accuracy of UnderSampling Model:", 1-InformationValue::misClassError(test$y, yhat.glm.under))
```

```{r}
fit.glm <- glm(y~., train, family="binomial", weights=model_weights)
yhat.glm <- predict(fit.glm, test, type="response")
cat("Accuracy of Balanced Model:", 1-InformationValue::misClassError(test$y, yhat.glm))
```


```{r fig.width=10, fig.height=4}
p1 <- MyConfusionMatrix(test$y, yhat.glm.unbalanced, title="Logistic Regression Unbalanced")
p2 <- MyConfusionMatrix(test$y, yhat.glm.under, title="Logistic Regression UnderSampling")
p3 <- MyConfusionMatrix(test$y, yhat.glm, title="Logistic Regression Balanced")
p1+p2+p3
```
...
```{r}
InformationValue::plotROC(test$y, yhat.glm)
```
...
```{r}
fit.pca <- glm(y~., train.pca, family="binomial",  weights=model_weights)

yhat.pca <- predict(fit.pca, test.pca, type="response")

MyConfusionMatrix(test.pca$y, yhat.pca, title = "Logistic Regression - PCA")
cat("Accuracy = ", 1-InformationValue::misClassError(test.pca$y, yhat.pca))
```
...
```{r}
train.tiny <- train %>% slice_sample(n=1e3) %>% ungroup() 
model_weights.tiny <- ifelse(train.tiny$y == "0",
                        (1/table(train.tiny$y)[1]) * 0.5, ### (1 / numero osservazioni classe 0) / 2
                        (1/table(train.tiny$y)[2]) * 0.5) ### (1 / numero osservazioni classe 1) / 2

fit.glm.tiny <- glm(y~int_rate, train.tiny, family="binomial", weights=model_weights.tiny)

glm.backwards = step(fit.glm.tiny, direction = "backward", trace=T, k=2)

fit.glm.backwards <- glm(glm.backward$formula, train, family = "binomial", weights=model_weights)

yhat.glm.backwards = predict(fit.glm.backwards, test)

MyConfusionMatrix(test$y, yhat.glm.backwards, title = "Logistic Regression - Backward Elimination")

cat("Accuracy = ", 1-InformationValue::misClassError(test$y, yhat.glm.backwards))
```

```{r}
fit.null.tiny <- glm(y~1, train.tiny, family="binomial", weights=model_weights.tiny)
glm.forward <- step(fit.null.tiny, direction = "forward", trace=T, k=2)
...
```{r}
InformationValue::plotROC(test$y, yhat.glm.backwards)
```
...
K <- 10
fit.ridge.cv <- cv.glmnet(X.train, train$y, weights= model_weights , alpha=0, family="binomial", nfolds = K, grouped = FALSE, type.measure = "class", parallel = T)
plot(fit.ridge.cv)
```
...
MyConfusionMatrix(test$y, yhat.ridge, threshold = 0.5)
...
```{r}
K <- 10
fit.lasso.cv <- cv.glmnet(X.train, train$y, weights=model_weights, alpha=1, family="binomial", nfolds = K, grouped = FALSE, type.measure = "class", parallel = T)
plot(fit.lasso.cv)
```
...
MyConfusionMatrix(test$y, yhat.lasso, threshold = 0.5)
...
{
  data <- data %>% group_by(y) %>% slice_sample(n=4e5) %>% ungroup() 
  x <- data %>% select(-y)
  y <- data$y
  tuning_data <- data %>% group_by(y) %>% slice_sample(n=1e5) %>% ungroup()
...
```{r}
importances <- as.data.frame(fit.rf.150t$importance) %>% arrange(-MeanDecreaseAccuracy)
plot(x = importances$MeanDecreaseAccuracy, y=rownames(importances))
varImpPlot(fit.rf.150t, type=1, scale=F)
abline(v=0)
```
...
fit.rf.caret = train(y ~ ., train %>% group_by(y) %>% slice_sample(n=1e4) %>% ungroup(), method="rf",
                     ntree = 100,
...
```{r}
fit.rf.caret2 = train(y ~ ., train %>% group_by(y) %>% slice_sample(n=1e4) %>% ungroup(), method="rf",
                      ntree = 100,
                      tuneGrid=expand.grid(.mtry=seq(
                        fit.rf.caret$bestTune$mtry-2,
                        fit.rf.caret$bestTune$mtry+2,
                        1)),
                      trControl = trainControl(method="repeatedcv", number = K, repeats=3, search="grid"))
...
```{r}
fit.rf.caret.final <- train(y ~ ., train %>% group_by(y) %>% slice_sample(n=1e5) %>% ungroup(), method="rf", ntree = 200,
               tuneGrid=expand.grid(.mtry=fit.rf.caret2$bestTune$mtry),
               trControl = trainControl(method="repeatedcv", number = K, repeats=3, search="grid"))
```
