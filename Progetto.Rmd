...
yhat.glm <- predict(fit.glm, test.balanced)

InformationValue::confusionMatrix(ystar, yhat.glm-1)
```
...
InformationValue::confusionMatrix(ytest.pca, yhat.pca)
```
...
```{r}
fit.bss = regsubsets(y ~ ., train.balanced, really.big=T)

summary.bests <- summary(fit.bss)   #PER OTTENERE I RISULTATI

head(summary.bests$outmat, 10) #PER VEDERE SOLO UNA PARTE

plot(summary.bests$cp, xlab = "k", ylab = "Cp", type = "b")

plot(fit.bss, scale = "bic")
...
yhat.bestCp <- predict.regsubsets(fit.bss, newdata = test,
id = which.min(summary.bests$cp))
...
yhat.bestBIC <- predict.regsubsets(fit.bests, newdata = test,
id = which.min(summary.bests$bic))
