...
acc <- acc %>% 
  ...
  select(-num_sats, num_bc_sats) %>% mutate(y=as.numeric(as.character(y)))
```
...
InformationValue::confusionMatrix(test.balanced$y, yhat.glm-1)
```
...
```{r}
fit.bss = bestglm(train.balanced %>% select(-y) %>% cbind(train.balanced$y), family=binomial, IC="BIC")

yhat = predict(fit.bss$BestModel, test.balanced %>% select(-y))

InformationValue::plotROC(test.balanced$y, yhat)
```
