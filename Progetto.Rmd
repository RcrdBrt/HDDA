...
```{r}
train <- acc %>% group_by(y) %>% sample_n(1000) %>% ungroup()

train <- acc[tr_ids,]
test <- acc[-tr_ids,] %>% group_by(y) %>% sample_n(1000) %>% ungroup()
...
```{r}
tr_ids <- sample(c(1:nrow(acc_pca)), 1e5)

train <- acc_pca[tr_ids,]
test <- acc_pca[-tr_ids,] %>% group_by(y) %>% sample_n(1000) %>% ungroup()
