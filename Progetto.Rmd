...
acc <- acc %>%
  mutate(
    ...
    issue_d=as.numeric(as.POSIXct(lubridate::my(issue_d))),
...
```{r}
corrplot(cor(acc[,corr_variables]))
```
...
p.mat <- cor.mtest(acc[,corr_variables])
col <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
png(filename = "/Users/alessandro/Desktop/Corrplot_new.png", width = 900, height = 900, type = "cairo")
corrplot::corrplot(cor(acc[,corr_variables]), method="color", col=col(200),  
         type="upper", 
...
```{r}
tr_ids <- sample(c(1:nrow(acc)), 1e5)
te_ids <- sample(c(1:nrow(acc)) %notin% tr_ids, 1e5)

fit <- glm(y~., acc[tr_ids,] %>% select(-loan_status, -addr_state, -grade, -sub_grade, -emp_title) %>% mutate(y=ifelse(y=="Default",0,1)), family="binomial")

yhat <- predict(fit, acc[te_ids,] %>% select(-loan_status, -addr_state, -grade, -sub_grade, -emp_title))
ytest <- acc[te_ids, "y"]
