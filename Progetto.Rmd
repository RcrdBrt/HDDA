...
acc <- acc[!future_apply(acc, 1, function(x) any(x=="")),]
```
...
```{r}
acc.balanced100k <- acc %>% group_by(y) %>% sample_n(1e5) %>% ungroup()

train.balanced100k <- sample_frac(acc.balanced100k, 0.8)
test.balanced100k <- anti_join(acc.balanced100k,train.balanced100k)


fit.balanced100k <- glm(y~., train.balanced100k %>% select(-loan_status, -addr_state, -grade, -sub_grade, -emp_title) %>% mutate(y=ifelse(y=="Default",0,1)), family="binomial")

yhat.balanced100k <- predict(fit.balanced100k, test.balanced100k %>% select(-loan_status, -addr_state, -grade, -sub_grade, -emp_title))

ytest.balanced100k <- test.balanced100k$y

InformationValue::confusionMatrix(ytest.balanced100k, yhat.balanced100k)
```
...
```{r}
acc.balanced100k.pca <- acc_pca %>% group_by(y) %>% sample_n(1e5) %>% ungroup()

train.balanced100k.pca <- sample_frac(acc.balanced100k.pca, 0.8)
test.balanced100k.pca <- anti_join(acc.balanced100k.pca,train.balanced100k.pca)

fit.balanced100k.pca <- glm(y~., train.balanced100k.pca %>% mutate(y=ifelse(y=="Default",0,1)), family="binomial")

yhat.balanced100k.pca <- predict(fit.balanced100k.pca, test.balanced100k.pca)

ytest.balanced100k.pca <- ifelse(test.balanced100k.pca$y == "Default", 0, 1)

InformationValue::confusionMatrix(ytest.balanced100k.pca, yhat.balanced100k.pca)
```

```{r}
InformationValue::plotROC(ytest.balanced100k.pca, yhat.balanced100k.pca)
```


## Best Subset Selection (BSS)
```{r}
acc.balanced10k = acc.balanced100k %>%group_by(y) %>% sample_n(1e4) %>% ungroup()

train.balanced10k = sample_frac(acc.balanced10k, 0.8)
test.balanced10k.pca <- anti_join(acc.balanced10k,train)

fit.balanced10k.bss = regsubsets(y ~ ., train.balanced10k)
```
