...
```{r}
set.seed(1000)
Xlm <- model.matrix(fit.glm)[,-1]
fit.ridge <- glmnet(Xlm, train.balanced$y, alpha = 0, family="binomial")    # alpha=0: ridge
...
set.seed(1000)
K <- 10
fit.ridge.cv <- cv.glmnet(Xlm, train.balanced$y, alpha=0, nfolds = K, grouped = FALSE, family="binomial", type.measure = "auc")
plot(fit.ridge.cv)
abline(v=log(fit.ridge.cv$lambda.min))
```
...
set.seed(1000)
K <- 10
cv.fit.lasso <- cv.glmnet(Xlm,y,alpha=1, nfolds = K, grouped = FALSE, family="binomial", type.measure="auc" ,parallel=TRUE)
plot(cv.fit.lasso)
abline(v=log(cv.fit.lasso$lambda.min))
coef(cv.fit.lasso)
```

```{r}
set.seed(1000)
yhat.lasso <- predict(cv.fit.lasso, newx = model.matrix(lm(y~., test.balanced))[,-1], s = "lambda.min")
plotROC(test.balanced$y, yhat.lasso)
confusionMatrix(
  test.balanced$y,
  yhat.lasso,
...
hist(as.numeric(acc_surv[acc_surv$y==1, "actual_duration_to_term"]), col = "blue")
hist(as.numeric(acc_surv[acc_surv$y==0, "actual_duration_to_term"]), col="red",add =T)
