...
```{r}
yhat.glm <- predict(fit.glm, test.balanced, type="response")

confusionMatrix(test.balanced$y, yhat.glm)
```

```{r}
InformationValue::plotROC(test.balanced$y, yhat.glm)
```
```{r}
confusionMatrix(test.balanced$y, yhat.glm, threshold = 0.8)
```
```{r}
precision(test.balanced$y, yhat.glm, threshold = 0.8)
```
...
print(pca.res$sdev) # si prendono quelle > 1
```
...
yhat.pca <- predict(fit.pca, test.balanced.pca, type="response")
...
```
```{r}
precision(ytest.pca, yhat.pca, threshold=0.8)
```


```{r}
InformationValue::plotROC(ytest.pca, yhat.pca)
```
Cerchiamo quindi altri metodi blah vlah

## Best Subset Selection (BSS)

```{r}
set.seed(1000)
fit.glm.backwards = step(fit.glm, direction = "backward", trace=T)

yhat.glm.backwards = predict(fit.glm.backwards, test.balanced %>% select(-y))

InformationValue::plotROC(test.balanced$y, yhat.glm.backwards)
```

```{r}
precision(test.balanced$y, yhat.glm.backwards)
```
...
fit.ridge <- glmnet(Xlm.train, train.balanced$y, alpha = 0, family="binomial", nlambda = 100)    # alpha=0: ridge
...
set.seed(1000)
K <- 10
fit.ridge.cv <- cv.glmnet(Xlm.train, train.balanced$y, alpha=0, nfolds = K, grouped = FALSE, family="binomial")
...
yhat.ridge.cv.1se = predict(fit.ridge.cv$glmnet.fit, Xlm.test, s = "lambda.1se")

confusionMatrix(test.balanced$y, yhat.ridge.cv.1se, threshold = 0.8)
```


```{r}
InformationValue::plotROC(test.balanced$y, yhat.ridge.cv.1se)
```


```{r}
precision(test.balanced$y, yhat.ridge.cv.1se, threshold = 0.8)
```
...
set.seed(1000)
K <- 10
fit.lasso.cv <- cv.glmnet(Xlm, train.balanced$y, alpha=1, nfolds = K, grouped = FALSE, family="binomial")
plot(fit.lasso.cv)
```
...
```{r}
InformationValue::plotROC(test.balanced$y, yhat.lasso.cv.1se)
```

```{r}
precision(test.balanced$y, yhat.lasso.cv.1se, threshold = 0.8)
```
...
```{r}
customTuneRF = function (x, y, ntree) 
{
  e = tuneRF(x, y, trace=T)
  
  randomForest(x, y, mtry=e[which.min(e[, 
      2]), 1], ntree=ntree, importance=T)
}
```
...
set.seed(1000)
fit.rf.bestMtry.500t = customTuneRF(train.balanced.tiny %>% select(-y), train.balanced.tiny$y, ntree=500)
```


```{r}
varImpPlot(fit.rf.bestMtry.500t, type=1)
```

```{r}
plot(fit.rf.bestMtry.500t)
```


```{r}
set.seed(1000)
fit.rf.bestMtry.250t = customTuneRF(train.balanced.tiny %>% select(-y), train.balanced.tiny$y, ntree=250)
```


```{r}
varImpPlot(fit.rf.bestMtry.250t, type=1)
```


```{r}
plot(fit.rf.bestMtry.250t)
```
...
```{r}
yhat.rf = predict(fit.rf.bestMtry, test.balanced.tiny %>% select(-y), type="prob", norm.votes=T)

InformationValue::confusionMatrix(test.balanced.tiny$y, yhat.rf[,2], threshold = 0.8)
```


```{r}
InformationValue::plotROC(test.balanced.tiny$y, yhat.rf[,2])
```

```{r}
precision(test.balanced.tiny$y, yhat.rf[,2], threshold = 0.8)
```
...
fit.rf.100trees = train(y ~ ., train.balanced.tiny, method="rf", ntree = 100,
               tuneGrid=expand.grid(.mtry=seq(1, 29, 2)),
               trControl = trainControl(method="cv", number = K, search="grid"))
fit.rf.1000trees = train(y ~ ., train.balanced.tiny, method="rf", ntree = 1000,
               tuneGrid=expand.grid(.mtry=seq(1, 29, 2)),
               trControl = trainControl(method="cv", number = K, search="grid"))
stopCluster(cluster)
plot(fit.rf)
...
```{r}
plotCalibration(score,times=0.5, cens.method="local",method="quantile", q=20, auc.in.legend = F, xlim = c(0,0.40),ylim=c(0,0.40))
