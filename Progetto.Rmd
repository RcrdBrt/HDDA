...
cat("Accuracy of Balanced Model:", 1-misClassError(test$y, yhat.glm))
```
...
plot(x=1:length(pca.res$sdev), y=pca.res$sdev, type="p", col=ifelse(pca.res$sdev>=1,"red","blue") , xlab="Component", ylab="Standard Deviation") # si prendono quelle > 1
abline(h=1, col="red")
...
customTuneRF<- function (data, ntree, mtryStart){
  ...
  tune <- tuneRF(tuning_x, tuning_y, trace=T, strata=tuning_y, mtryStart=mtryStart, ntreeTry = 100)
...
```{r}
fit.rf.150t = customTuneRF(train, ntree=250, mtryStart=7)
```
...
prep_surv <- function(d, plots=T){
  ...
  if (plots){
    ...
    pie <- piechart(d_pie, "y") +
      scale_fill_manual(labels=c("Regular Payment", "Default", "Early Payment"), values=c("darkgreen", "darkred", "darkgray"))
  }
  d <- d %>%
  ...
  mutate(actual_duration_to_term = ifelse(actual_duration_to_term > 1, 1, actual_duration_to_term)) %>%
    # ignoro gli early payment e i pagamenti in ritardo, settando il tempo a 1 per loro, intendendo che questi
    # prestiti sono sopravvissuti fino alla fine.
...
set.seed(1000)
train_surv <- train_surv %>% group_by(y) %>% slice_sample(n=min(table(train_surv$y))) %>% ungroup()
...
train_surv[, new_numeric_variables[new_numeric_variables %in% names(acc_surv)]] <- scale(model_acc[, new_numeric_variables[new_numeric_variables %in% names(acc_surv)]])
```
