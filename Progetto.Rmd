...
cat("Accuracy at 95% C.I for a 20-fold Crossvalidation:", round(mean(accs),5), "±", round(1.960*sd(accs)/sqrt(nrow(train)/K),5),"\n")

cat("Precision at 95% C.I for a 20-fold Crossvalidation:", round(mean(precs),5), "±", round(1.960*sd(precs)/sqrt(round(nrow(train))/K),5))
```
...
Si osserva come il Random Forest minimizzi l'errore di previsione con l'utilizzo di 4 regressori (campionati causalmente per ogni albero). Il sottocampionamento dell'insieme dei predittori permette di ottenere alberi meno correlati tra loro, con l'effetto di contenere così la varianza e quindi l'errore di previsione
...
L'ultimo modello presentato è il Modello di Regressione di Cox, appartenente alla classe dei Modelli di Sopravvivenza. Esso nasce nel contesto dell'Analisi della Sopravvivenza, in cui si studiano gli effetti di trattamenti medici o di fattori di rischio specifici. Questo tipo di analisi ricerca una stima della *"Sopravvivenza"* dei pazienti in un determinato periodo di osservazione (la parola *"Sopravvivenza"* non assume necessariamente un significato letterale ma a seconda del contesto indica l'assenza di uno specifico evento avverso, ad esemoio se si studia il beneficio dell'impianto di una protesi l'evento avverso può essere la frattura dell'osso). \
...
- Il Default del prestito è l'evento avverso;
...
- È presente per costruzione un time-point fisso all'evento, ovvero il termine del prestito, oltre il quale non si hanno più dati. Fanno eccezione i prestiti ripagati in ritardo che complicano notevolmente l'analisi, per questi si può assumere che siano stati pagati al termine come semplificazione.
...
ggsurvplot(surv.cox, xlab = "Years", legend= "none", ggtheme = theme_minimal())
...
Ci si aspetta che il valore atteso dei residui sia 0, ne segue che l'andamento dei residui rispetto al tempo deve quantomeno essere costante. Si plottano i residui Schoenfeld standardizzati.
...
Forse si riesce a fare anche i calibration plot.
...
Occorre quindi considerare come i valori predetti dai modelli non possano essere correttamente interpretati come delle probabilità, ma semplicemente come dei punteggi di confidenza.
